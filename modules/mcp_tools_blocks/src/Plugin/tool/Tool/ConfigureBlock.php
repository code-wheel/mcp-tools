...
#[Tool(
  ...
  input_definitions: [
    'block_id' => new InputDefinition(
      ...
      description: new TranslatableMarkup('Config entity ID of placed block from PlaceBlock or ListRegions.'),
      required: TRUE,
    ),
    'region' => new InputDefinition(
      data_type: 'string',
      label: new TranslatableMarkup('Move to Region'),
      description: new TranslatableMarkup('New region to move block to. Get valid values from ListRegions.'),
      required: FALSE,
    ),
    'weight' => new InputDefinition(
      data_type: 'integer',
      label: new TranslatableMarkup('Weight'),
      description: new TranslatableMarkup('New sort order within region. Lower = higher position.'),
      required: FALSE,
    ),
    'label' => new InputDefinition(
      data_type: 'string',
      label: new TranslatableMarkup('Block Label'),
      description: new TranslatableMarkup('New block title shown to users.'),
      required: FALSE,
    ),
    'label_display' => new InputDefinition(
      data_type: 'string',
      label: new TranslatableMarkup('Label Display (visible/hidden)'),
      description: new TranslatableMarkup('Show or hide block title: "visible" or "hidden".'),
      required: FALSE,
    ),
    'status' => new InputDefinition(
      data_type: 'boolean',
      label: new TranslatableMarkup('Enable/Disable Block'),
      description: new TranslatableMarkup('True to enable, false to disable. Disabled blocks are hidden.'),
      required: FALSE,
    ),
    'visibility' => new InputDefinition(
      data_type: 'map',
      label: new TranslatableMarkup('Visibility Conditions'),
      description: new TranslatableMarkup('Update visibility rules. Replaces existing conditions.'),
      required: FALSE,
    ),
    'settings' => new InputDefinition(
      data_type: 'map',
      label: new TranslatableMarkup('Block Settings'),
      description: new TranslatableMarkup('Plugin-specific settings to update.'),
      required: FALSE,
    ),
  ],
  output_definitions: [
    'block_id' => new ContextDefinition(
      data_type: 'string',
      label: new TranslatableMarkup('Block ID'),
      description: new TranslatableMarkup('ID of the configured block.'),
    ),
    'updated' => new ContextDefinition(
      data_type: 'list',
      label: new TranslatableMarkup('Updated Fields'),
      description: new TranslatableMarkup('List of fields that were actually changed.'),
    ),
